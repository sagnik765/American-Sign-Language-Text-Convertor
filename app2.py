...
class Application:
    ...
    def video_loop(self):
        ...
        if ok:
            ...
            cv2image_temp = cv2.resize(cv2image, (310, 310))
            ...
            cv2image_temp = cv2.cvtColor(cv2image_temp, cv2.COLOR_BGR2RGBA)
            x1_t = int(0.5 * cv2image_temp.shape[1])
            y1_t = 0
            x2_t = cv2image_temp.shape[1]
            y2_t = int(0.5 * cv2image_temp.shape[1])
            cv2.rectangle(cv2image_temp, (x1_t + 5, y1_t), (x2_t, y2_t - 5), (128, 0, 0), 5)
            self.current_image = Image.fromarray(cv2image_temp)
            ...
            self.predicts = self.hs.suggest(self.word)
